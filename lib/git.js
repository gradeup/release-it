...
class Git {
  ...
  tag({ name = this.options.tagName } = {}) {
    return this.shell.runTemplateCommand(`git tag ${name}`, Shell.writes);
  }
  ...
  async push({ name = this.options.tagName } = {}) {
    ...
    return this.shell.run(`git push ${upstream} ${name}`, Shell.writes);
  }
}
